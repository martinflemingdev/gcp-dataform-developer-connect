...
![Developer Connect GitHub connection setup](./01_github_connection.png)
...
| Consuming project | `cdmc-data` |

The GitHub repository was:

```text
https://github.com/martinflemingdev/gcp-dataform-developer-connect
```
...
That final resource is the important one. The Dataform repository lives in `cdmc-data`, but its Git remote is backed by a Developer Connect GitRepositoryLink in `axial-life-395119`.

![Consumer project Dataform repository using the platform GitRepositoryLink](./05_consumer-project-uses-platform-connection-link.png)
...
![GitHub App configuration - general settings](./06_1_GitHub-App-config.png)

![GitHub App configuration - webhook settings](./06_2_GitHub-App-config.png)

![GitHub App configuration - private key and IP allow list](./06_3_GitHub-App-config.png)
...
![BigQuery unified API prompt](./04_1_consumer-project-needs-bq-unified-api.png)

![Developer Connect API prompt](./04_2_consumer-project-needs-developer-connect-api.png)
...
Then I tested the part the console did not make obvious: a Dataform repository in `cdmc-data` using a GitRepositoryLink in `axial-life-395119`.
...
metadata:
  name: cdmc-data-dataform-developerconnect-token-accessor
spec:
  forProvider:
    project: axial-life-395119
...
kind: ProjectIAMMember
metadata:
  name: cdmc-data-dataform-developerconnect-git-proxy-user
spec:
  forProvider:
    project: axial-life-395119
...
![Dataform service agent needs roles](./04_3_dataform-sa-needs-roles.png)

### 2. The IaC identity needs permission to create Dataform repositories in the consuming project

Crossplane also needed permission to create the Dataform repository in `cdmc-data`.

```yaml
apiVersion: cloudplatform.gcp.upbound.io/v1beta2
kind: ProjectIAMMember
metadata:
  name: crossplane-provider-dataform-admin-cdmc-data
spec:
  forProvider:
    project: cdmc-data
    role: roles/dataform.admin
    member: serviceAccount:crossplane@axial-life-395119.iam.gserviceaccount.com
```
